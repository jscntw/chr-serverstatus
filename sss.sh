...
install_agent() {
    ...
    echo -e "> 从仓库同步最新的 client-linux.py..."
    ...
    for proto in "v4" "v6"; do
        svc_name="sss-agent"
        [ "$proto" == "v6" ] && svc_name="sss-agent-v6"
        
        echo -e "> 正在配置 ${svc_name} 服务..."
        
        # 统一下载同一个服务模板
        wget -qO /etc/systemd/system/${svc_name}.service ${GITHUB_RAW_URL}/sss-agent.service
        
        # 动态替换参数
        # 注意：这里会自动将用户名变为 user_v4 和 user_v6，配合 client-linux.py 自动识别
        sed -i "s/sss_host/${s_host}/" /etc/systemd/system/${svc_name}.service
        sed -i "s/sss_user/${s_user}_${proto}/" /etc/systemd/system/${svc_name}.service
        sed -i "s/sss_pass/${s_pass}/" /etc/systemd/system/${svc_name}.service
        
        # 如果是 v6，修改 Service 的 Description 以便在 systemctl status 中区分
        if [ "$proto" == "v6" ]; then
            sed -i "s/Agent IPv4/Agent IPv6/" /etc/systemd/system/${svc_name}.service
        fi
    ...
    echo -e "${green}双栈 Agent 已启动成功！${plain}"
    echo -e "已自动创建两个节点：${yellow}${s_user}_v4${plain} 和 ${yellow}${s_user}_v6${plain}"
    echo -e "提示：请确保服务端 config.json 中已手动添加这两个用户名。"
}
...
show_menu() {
    clear
    echo -e "    ${green}Server Status 全能管理脚本 (Dockge 适配版)${plain}"
    echo -e "    --- https://github.com/jscntw/serverstatus ---"
    echo -e ""
    echo -e "    ${yellow}服务端 (Master)：${plain}"
    echo -e "    ${green}1.${plain}  安装服务端 (直接部署到 /opt/stacks/sss)"
    echo -e "    ${green}2.${plain}  管理节点 (添加/删除/查看)"
    echo -e ""
    echo -e "    ${yellow}受控端 (Agent)：${plain}"
    echo -e "    ${green}3.${plain}  安装双栈监控 (IPv4 + IPv6 一键部署)"
    echo -e ""
    echo -e "    ${yellow}通用：${plain}"
    echo -e "    ${green}4.${plain}  完全卸载"
